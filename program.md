...
## Model Optimization Phase — CRITICAL FINDINGS

### M3 Ultra is COMPUTE-BOUND (not memory-bound)
Post-training optimizations have ZERO effect on inference speed:
- Palettization (2/4/6-bit): 35ms → 35ms (no change)
- INT8 quantization: 35ms → 42ms (SLOWER + quality fail)
- Weight pruning: 35ms → 35ms (no change)
- Combined palettize+prune: 35ms → 35ms (no change)

**Only reducing actual FLOPs speeds up inference.**

### Channel-Slimmed UNet Models (WORKING!)
Created by `channel_slim.py` and `channel_slim_extreme.py`. Knowledge distillation
from the original 328M-param UNet to smaller architectures.

Current pipeline uses `unet_sdxs_512_slim_10.mlpackage` (4.7M params, 9.4MB).
Benchmark: **15.7ms / 63.9 FPS at full 512x512** (all quality gates pass).

Available variants in `coreml_models/`:
| Model | Channels | Params | Size | Speed (512) |
|-------|----------|--------|------|-------------|
| original | [320,640,1280] | 328M | 626MB | 35.0ms |
| slim_80 | [256,512,1024] | 213M | 406MB | 28.9ms |
| slim_50 | [160,320,640] | 86M | 165MB | 21.5ms |
| slim_40 | [128,256,512] | 56M | 108MB | 18.2ms |
| slim_30 | [96,192,384] | 33M | 63MB | 18.2ms |
| slim_20 | [64,128,256] | 16M | 30MB | 18.8ms |
| slim_15 | [64,96,192] | 10M | 20MB | 18.2ms |
| **slim_10** | **[32,64,128]** | **4.7M** | **9.4MB** | **15.7ms** |
| slim_asym | [128,192,256] | 23M | 44MB | 16.3ms |
| slim_wide | [192,256,384] | 45M | 86MB | 16.3ms |

### Pipeline Stage Breakdown (slim_10 UNet, 512x512)
```
preprocess:  0.33ms (2%)
VAE encode:  5.87ms (37%)
UNet:        ~3ms   (19%) ← no longer bottleneck
VAE decode:  7.03ms (44%) ← THE bottleneck
postprocess: 0.07ms (0%)
```

### VAE is the bottleneck
The TinyVAE (taesd) is only 2.4M params but processes 512x512 pixels.
VAE encode+decode = ~13ms = 81% of pipeline. Optimization options:
- Lower-res encoding (256→2.4ms, 128→1.2ms) + latent upsample
- Lower-res decoding (256→2.2ms, 128→0.9ms) + cv2 upscale
- CPU_AND_GPU is optimal (ALL/ANE are slower for VAE)

### Fast Pipeline (pipeline_fast.py)
Uses reduced VAE resolution with cv2 upscale to 512x512 output:
- 256 VAE + slim_10: **8.6ms / 116 FPS** (near-identical quality)
- 128 VAE + slim_10: **6.5ms / 154 FPS** (softer, acceptable for VJ)
- 64 VAE + slim_10: **6.2ms / 160 FPS** (blurry)
- 32 VAE + slim_10: **5.3ms / 189 FPS** (too blurry)

### Current Optimization Focus
You should optimize pipeline.py at **full 512x512** (no resolution tricks).
Current best: 15.7ms / 63.9 FPS with slim_10 UNet.

Remaining optimization axes:
1. Pipeline parallelism (overlap pre-process with previous frame's post-process)
2. Buffer optimization (eliminate remaining copies)
3. CoreML prediction options
4. Fuse pre/post processing into fewer cv2 calls
5. Profile and eliminate Python overhead in the hot loop

### What NOT to do
- Do NOT lower RENDER_SIZE below 512 (benchmark enforces this)
- Do NOT skip VAE encoder or decoder
- Do NOT remove the UNet from the pipeline
- Do NOT try palettization/INT8/pruning (proven useless on M3 Ultra)
- Do NOT try ANE for any model (proven slower than GPU)
