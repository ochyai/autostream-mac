...
Beyond pipeline code optimization, there are pre-optimized CoreML model variants
in `coreml_models/`. These were generated by `optimize_models.py`.

### Available Optimized UNet Models
- `unet_sdxs_512_pal6.mlpackage` — 6-bit palettization
- `unet_sdxs_512_pal4.mlpackage` — 4-bit palettization
- `unet_sdxs_512_pal2.mlpackage` — 2-bit palettization
- `unet_sdxs_512_int8.mlpackage` — INT8 linear quantization
- `unet_sdxs_512_pruned.mlpackage` — Magnitude-based weight pruning
- `unet_sdxs_512_pal4_pruned.mlpackage` — Pruning + 4-bit palettization

### How to Use
In `pipeline.py`, change the UNet model loading path to use an optimized variant:
```python
unet_path = os.path.join(COREML_DIR, f"{prefix}_pal4.mlpackage")
```

You MUST still pass all quality gates. Some aggressive quantizations (2-bit) may
fail quality checks — that's expected.

### Experiment Strategy
1. Try each UNet variant with the existing pipeline code
2. For each variant that passes quality, note the speed
3. Then optimize the pipeline code FOR that specific model variant
4. Palettized models may benefit from different compute unit settings

### Results File
`model_variants_results.tsv` has benchmark results for all variants.
`benchmark_all_variants.py` can re-run all variants in one go.
...
- Resolution is fixed at 512x512 — optimize the MODEL, not the resolution
